...
@Directive({
  selector: '[appHasRole]' // visible for users with certain roles
})
export class HasRolesDirective implements OnInit {
  ...
  ngOnInit(): void {
    if (this.user.roles.some(r => this.appHasRole.includes(r))) { this.viewContainerRef.createEmbeddedView(this.templateRef); }
